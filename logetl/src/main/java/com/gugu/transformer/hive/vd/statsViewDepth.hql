...
-- 5. 上传transformer-0.0.1.jar到hdfs的/gugu/transformer文件夹中
-- 6. 创建hive的function
#create function platform_convert as 'com.sxt.transformer.hive.PlatformDimensionUDF' using jar 'hdfs://master/gugu/transformer/transformer-0.0.1.jar'
CREATE FUNCTION date_convert as 'com.gugu.transformer.hive.DateDimensionUDF' using jar 'hdfs://master/gugu/transformer-0.0.1.jar';

-- 7. hql编写(统计用户角度的浏览深度)<注意：时间为外部给定>
from (
SELECT pl,from_unixtime(cast(s_time/1000 as bigint), 'yyyy-MM-dd') as day , u_id (case when count(p_url) = 1 then "pv1" when count(p_url) = 2 then "pv2" when count(p_url)=3 then "pv3" when count(p_url) =4 then "pv4" when count(p_url) >= 5 and count(p_url) <10 then "pv5_10" when count(p_url) >= 10 and count(p_url) <30 then "pv10_30" when count(p_url) >=30 and count(p_url) <60 then "pv30_60"  else 'pv60_plus' end) as pv
from event_logs
where en='e_pv' and p_url is not null and  s_time >= unix_timestamp('2016-06-08','yyyy-MM-dd')*1000 and s_time < unix_timestamp('2016-06-09','yyyy-MM-dd')*1000
group by pl, from_unixtime(cast(s_time/1000 as bigint),'yyyy-MM-dd'), u_ud
) as tmp
insert overwrite table stats_view_depth_tmp select pl,day,pv,count(distinct u_ud) as ct when u_ud is not null group by pl,day,pv;
...
-- 9. sqoop脚步编写(统计会话角度)
sqoop export --connect jdbc:mysql://master:3306/report --username root --password root --table stats_view_depth --export-dir /hive/bigdater.db/stats_view_depth/*  --input-fields-terminated-by "\\01" --update-mode allowinsert --update-key platform_dimension_id,kpi_dimension_id
