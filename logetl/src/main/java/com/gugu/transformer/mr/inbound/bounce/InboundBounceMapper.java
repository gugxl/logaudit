...
import com.gugu.transformer.util.UrlUtil;
...
/**
 * @author gugu
 * @Classname InboundBounceMapper
 * @Description 计算外链跳出会话的会话个数
...
public class InboundBounceMapper extends TransformerBaseMapper<StatsInboundBounceDimension, IntWritable> {
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        // 过滤
        if (StringUtils.isBlank(platform) || StringUtils.isBlank(serverTime) || StringUtils.isBlank(referrerUrl) || StringUtils.isBlank(sessionId) || !StringUtils.isNumeric(serverTime.trim())) {
            this.filterRecords++;
...
// 输出定义
        ...
        StatsCommonDimension statsCommon = statsInboundBounceDimension.getStatsCommon();
        statsCommon.setDate(dateDimension);
        statsCommon.setKpi(this.inboundBounceKpi);
        this.statsInboundBounceDimension.setSid(sessionId);
     ...
     * @param host
     * @return int
     * @Description 根据url的host来获取不同的inbound
