...
/**
 * @author gugu
 * @Classname InboundBounceReducer
 * @Description 统计外链跳出会话的reducer类
    ...
    protected void reduce(StatsInboundBounceDimension key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
        ...
        for (IntWritable value :values){
            ...
            if (curInboundId != InboundBounceMapper.DEFAULT_INBOUND_ID){
                ...
                InboundBounceReduceValue inboundBounceReduceValue = map.get(curInboundId);
                if (inboundBounceReduceValue == null){
                    inboundBounceReduceValue = new InboundBounceReduceValue(0);
                    map.put(curInboundId, inboundBounceReduceValue);
                }
            }
            // 如果是一个新的会话，那么更新会话
