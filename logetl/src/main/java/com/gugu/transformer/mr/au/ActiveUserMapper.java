...
public class ActiveUserMapper extends TableMapper<StatsUserDimension, TimeOutputValue> {
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        // 过滤无效数据
        if (StringUtils.isBlank(uuid) || StringUtils.isBlank(platform) || StringUtils.isBlank(platform) || StringUtils.isBlank(serverTime)|| !StringUtils.isNumeric(serverTime.trim())){
        ...
        for (PlatformDimension  platformDimension:platforms){
            ...
            // 输出browser维度统计
            statsCommon.setKpi(activeUserBrowserKpi);
            for(BrowserDimension browserDimension :browsers){
                this.outputKey.setBrowser(browserDimension);// 设置对应的browsers
                context.write(this.outputKey,this.outputValue);
            }
        }
