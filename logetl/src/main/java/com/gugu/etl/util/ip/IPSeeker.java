 ...
 * @author gugu
 * @Classname IPSeeker
 * @Description 用来读取QQwry.dat文件，以根据ip获得好友位置，QQwry.dat的格式是 一. 文件头，共8字节 1. 第一个起始IP的绝对偏移， 4字节
...
public class IPSeeker {
    ...
    /**
     * @return com.gugu.etl.util.ip.IPSeeker
     * @Description 单一实例
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param s 地点子串
     * @return java.util.List 包含IPEntry类型的List
     * @Description 给定一个地点的不完全名字，得到一系列包含s子串的IP范围记录
     * @params
     * @auther gugu
     */
...
//            如果temp不等于-1，读取IP的地点信息
            if (temp != -1) {
                IPLocation ipLocation = getIPLocation(temp);
//                判断是否这个地点里面包含了s子串，如果包含了，添加这个记录到List中，如果没有，继续
                if (ipLocation.country.indexOf(s) != -1 || ipLocation.area.indexOf(s) != -1) {
                    IPEntry ipEntry = new IPEntry();
                    ipEntry.country = ipLocation.country;
                    ipEntry.area = ipLocation.area;
//                    得到起始IP
                    readIP(offset - 4, b4);
                    ipEntry.beginIp = IPSeekerUtils.getIpStringFromBytes(b4);
//                    得到结束IP
                    readIP(temp, b4);
                    ipEntry.endIp = IPSeekerUtils.getIpStringFromBytes(b4);
//                    添加该记录
                    ret.add(ipEntry);
                }
            }
...
//                    添加该记录
     ...
     * @param offset
     * @return int
     * @Description 从内存映射文件的offset位置开始的3个字节读取一个int
     * @params
     * @auther gugu
     */
    ...
    /**
     * @return int
     * @Description 从内存映射文件的当前位置开始的3个字节读取一个int
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param ip ip的字节数组形式
     * @return java.lang.String ip的字节数组形式
     * @Description 根据IP得到国家名
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param ip IP的字符串形式
     * @return java.lang.String 国家名字符串
     * @Description 根据IP得到国家名
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param ip ip的字节数组形式
     * @return java.lang.String 地区名字符串
     * @Description 根据IP得到地区名
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param ip 要查询的IP
     * @return com.gugu.etl.util.ip.IPSeeker.IPLocation
     * @Description 根据ip搜索ip信息文件，得到IPLocation结构，所搜索的ip参数从类成员ip中得到
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param offset
     * @return long 读取的long值，返回-1表示读取文件失败
     * @Description 从offset位置读取4个字节为一个long，因为java为big-endian格式，所以没办法 用了这么一个函数来做转换
     * @params
     * @auther gugu
     */
     ...
     * @param offset
     * @return long 读取的long值，返回-1表示读取文件失败
     * @Description 从offset位置读取3个字节为一个long，因为java为big-endian格式，所以没办法 用了这么一个函数来做转换
     * @params
     * @auther gugu
     */
    ...
    /**
     * @return long
     * @Description 从当前位置读取3个字节转换成long
     * @params
     * @auther gugu
     */
     ...
     * @param ip
     * @return void
     * @Description 从offset位置读取四个字节的ip地址放入ip数组中，读取后的ip为big-endian格式，但是
     ...
     * @return int 相等返回0，ip大于beginIp则返回1，小于返回-1。
     * @Description 把类成员ip和beginIp比较，注意这个beginIp是big-endian的
     * @params
     * @auther gugu
     */
     ...
     * @param b1
     * @param b2
     * @return int 若b1大于b2则返回1，相等返回0，小于返回-1
     * @Description 把两个byte当作无符号数进行比较
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param ip 要查询的IP
     * @return long 如果找到了，返回结束IP的偏移，如果没有找到，返回-1
     * @Description 这个方法将根据ip的内容，定位到包含这个ip国家地区的记录处，返回一个绝对偏移 方法使用二分法查找。
     * @params
     * @auther gugu
     */
...
//            log.debug(Utils.getIpStringFromBytes(b));
     ...
     * @param end
     * @return long
     * @Description 得到begin偏移和end偏移中间位置记录的偏移
     * @params
     * @auther gugu
     */
     ...
     * @param offset
     * @return com.gugu.etl.util.ip.IPSeeker.IPLocation
     * @Description 给定一个ip国家地区记录的偏移，返回一个IPLocation结构
     * @params
     * @auther gugu
     */
...
//            读取地区标志
     ...
     * @Description 从offset偏移开始解析后面的字节，读出一个地区名
     * @params
     * @auther gugu
     */
    ...
    /**
     * @param offset
     * @return java.lang.String 读取的字符串，出错返回空字符串
     ...
     * @param offset
     * @return java.lang.String
     * @Description 从内存映射文件的offset位置得到一个0结尾字符串
     * @params
     * @auther gugu
     */
    ...
    public static class IPSeekerUtils {
         ...
         * @return java.lang.String 转换编码后的字符串，失败返回原始字符串
         * @Description 对原始字符串进行编码转换，如果失败，返回原始的字符串
         * @params
         * @auther gugu
         */
